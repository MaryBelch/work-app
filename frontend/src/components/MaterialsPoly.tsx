...
export default function MaterialsPoly() {
  ...
  const load = async () => {
    try {
      setLoading(true);
      const data = await getMaterials('poligraphy');
      setItems(data);
    } catch (err: any) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing) {
        await updateMaterial('poligraphy', editing.id, data);
      } else {
        await createMaterial('poligraphy', data);
      }
      setShowModal(false);
      setError('');
      await load();
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await deleteMaterial('poligraphy', id);
      await load();
    } catch (err: any) {
      setError(err.message);
    }
  ...
  return (
    <div>
      <div className="tab-header">
        <div className="tab-title">📦 Материалы (полиграфия)</div>
          ...
          <div className="modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group">
              ...
              <input
                ...
                placeholder="Например: Бумага мелованная 350г"
                value={form.name}
